...
try:
    ...
    print("✅ BOT_TOKEN loaded from config.py")
except Exception:
    BOT_TOKEN = os.environ.get("BOT_TOKEN")
    if not BOT_TOKEN:
        print("❌ BOT_TOKEN not found in config.py or environment")
        exit(1)
...
# Database Path
DB_PATH = "bot_ids.db"
file_status = {}  # Dictionary to store status: pending / approved / rejected
...
# ⛔ BLOCK non-approved files for run/download/log
    if ":" in call.data:
        parts = call.data.split(":")
        if len(parts) >= 3:
            action, file_user_id, filename = parts[0], parts[1], ":".join(parts[2:])
            if action in ["run", "download", "log"]:
                if file_status.get(filename, {}).get("status") != "approved":
                    bot.answer_callback_query(
                        call.id,
                        "⛔ This file is not approved yet.\n💬 Contact @SP1DEYYXPR1ME",
                        show_alert=True
                    )
                    return    
